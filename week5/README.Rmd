...
```
The station that is closest to the temp median is `r median_temp_station$USAFID`.
The station that is closest to the wind speed median is `r median_wind.sp_station$USAFID`.
The station that is closest to the atmospheric pressure median is `r median_atm.press_station$USAFID`.
...
```{r}
#calculate the station closest to the true median of both wind speed and temp
median_eudist_station <- station_averages[order(eudist)][1]
median_eudist_station
```

The station that is closest to the shared median of wind speed and temperature is is `r median_eudist_station$USAFID`.
...
```{r}
#creating a new matrix "tab" that has counts the number of each low, mid, and high READING as "N_entries" and also counts the number of each STATION as N_stations.
tab <- met[, .(
  N_entries = .N,
...
STATUS: This question is not finished, and I need to figure out what's needed.
